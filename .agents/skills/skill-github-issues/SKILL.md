...
interfaces:
  - id: issue-request
    description: Issue action or prompt to create, update, or query.
    required: true
  - id: repository-context
    description: Target repository, issue number, or label context.
    required: true
  - id: issue-type
    description: Optional issue type or template preference.
    required: false
---
...
- **Determine `issue-request`**: Create, update, or query?
- **Gather `repository-context`**: Get repo info, existing labels, milestones if needed
- **Structure content**: Use appropriate template from [references/templates.md](http://references/templates.md)
- **Choose `issue-type`** when the issue should map to Bug, Feature, Task, or a comparable org-level classification.
...
Use `repository-context` to resolve the owner, repo, and any existing issue number, keep the requested action aligned to `issue-request`, and set `issue-type` whenever the organization exposes canonical GitHub issue types.
