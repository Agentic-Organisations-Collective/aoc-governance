...
- **Analyze** all workflow outputs and handoff dependencies for roundtrips
- **Identify** friction points where roles asked clarifying questions or rework was needed
...
- Review "Open Questions" sections across all workflow outputs
- Identify which questions caused delays or rework
- Track where feedback loops occurred (one role asked a clarifying question → `@requirements-engineer` → restart a downstream phase)

**System Instructions:**

- Reference the agent definition files: AGENTS.md, individual *.agent.md files
- Assess if workflow participants needed to deviate from instructions
...
- Phase 1 → Phase 2: Did `@architect` need to ask clarifying questions? How many iterations?
- Phase 2 → Phase 3: Did planning or `@test-engineer` need to re-read requirements?
- Phase 3-5 → Phase 6-7: Did `@code-reviewer` or `@requirements-auditor` find issues that should have been caught earlier?
- Any phase → Earlier phase: Were there loops requiring rework?

**Friction Point Analysis:**

- Which role output had the most "Open Questions"?
- Which transitions between roles caused delays?
...
- Did any workflow participant request clarification about their role?
- Did roles cross boundaries (e.g., `@architect` writing code, `@requirements-engineer` suggesting architecture)?
- Were there conflicts between roles in how they interpreted requirements?
- Did tool availability limit any role's effectiveness?
...
- Role boundary conflicts between workflow participants
...
- Patterns that emerged (e.g., a specific role frequently needed clarification about X)
...
| Phase | Role | Duration | Status | Issues |
...
| 3. Planning | planning or `@test-engineer` | [Est] | ✅ | [Issues if any] |

| 4. Execution | Coding | [Est] | ✅ | [Issues if any] |

| 5. Review | @code-reviewer | [Est] | ✅ | [Issues if any] |

| 6. Validation | @requirements-auditor | [Est] | ✅ | [Issues if any] |
...
- **@requirements-engineer → @architect**: [Smooth / Issues]
  - [Specific clarifications needed]
- **@architect → planning or `@test-engineer`**: [Smooth / Issues]
  - [Specific clarifications needed]
- **[Phase] → [Phase]**: [Description]

## Workflow Instructions Assessment
### Role Clarity
...
- [Role or agent implementation] should clarify: [Clarification needed]
- [Role or agent implementation] should include: [New tool or constraint needed]
...
- Improve handoff between [Role A] and [Role B]
- Add validation gate for [specific concern]

### Process Enhancements (Longer term)

- Consider adding parallel phase for [optimization]
- Revisit the implementation tool set for [capability]

## Positive Observations
### What Went Well

- [Smooth phase transition]
- [Excellent cross-role collaboration]
- [Good catch by a specific role or implementation]
...
## Metrics & Data
- Total phases: 6
- Total role handoffs: 5
- Roundtrips identified: [N]
- Open questions across all workflow outputs: [N]
- Rework cycles: [N]
...
- `@requirements-auditor` has completed validation
- Feature is ready for deployment or in post-review
- All workflow outputs are finalized
...
- `@supervisor` can reference role-boundary issues as process improvement suggestions
...
- **SYSTEMIC FOCUS** - Identify patterns across the process, not individual implementation issues
